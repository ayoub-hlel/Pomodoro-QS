...
QtObject {
    ...
    function seed() {
        ...
        var t4 = TaskDB.createTask(root.list.id, "Deploy to staging", 4200);
        TaskDB.updateTask(t4.id, {
            actual_time: 5000,
            is_completed: 0
        });
        TaskDB.setSubtask(t4.id, "Build", 1);
        TaskDB.setSubtask(t4.id, "Run migrations", 1);
